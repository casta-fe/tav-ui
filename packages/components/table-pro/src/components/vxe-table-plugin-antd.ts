...
function getOns(
  ...
  inputFunc?: (...arg: any[]) => any,
  changeFunc?: (...arg: any[]) => any
) {
  const { events } = renderOpts
  const modelEvent = getModelEvent(renderOpts)
  const changeEvent = getChangeEvent(renderOpts)
  const isSameEvent = changeEvent === modelEvent
  const ons: { [type: string]: (...arg: any[]) => any } = {}
  XEUtils.objectEach(events, (func: (...arg: any[]) => any, key: string) => {
...
function createExportMethod(getExportCellValue: (...arg: any[]) => any) {
  return function (params: VxeGlobalRendererHandles.ExportMethodParams) {
    const { row, column, options } = params
    return options && options.original
      ? XEUtils.get(row, column.property)
