...
const editorOptions = computed(() => {
  const options: EditorOptions = {
     ...
     */
    content_style: `
    body{width:auto !important;height:auto !important;margin:10px !important;}
      ...
      v.forEach((uploadedImage: FileUploadApiResponseRecord & { [key: string]: any }) => {
        editorInstanceRef?.undoManager?.transact?.(() =>
          editorInstanceRef?.insertContent(
            `<img class="file file-image" data-uploadvarsjson='${uploadedImage.uploadVarsJson}' src="${uploadedImage.url}" width="${uploadedImage.originalWidth}" height="${uploadedImage.originalHeight}" />`
          )
        )
      })
